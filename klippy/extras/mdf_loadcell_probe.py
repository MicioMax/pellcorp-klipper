...
class LoadCellProbeCommands:
    ...
    def _register_commands(self):
        ...
        gcode.register_command("LOAD_CELL_PROBE",
            self.cmd_LOAD_CELL_PROBE, desc=self.cmd_LOAD_CELL_PROBE_help)
    ...
    cmd_LOAD_CELL_PROBE_help = "Probe Z using the load cell"

    def cmd_LOAD_CELL_PROBE(self, gcmd):
        epos, is_good = self._tapping_move.run_tap(gcmd)
        if not is_good:
            raise gcmd.error("Load cell probe failed")
        gcmd.respond_info("Load cell probe z=%.6f" % (epos[2],))
...
class LoadCellPrinterProbe:
    def __init__(self, config):
        ...
        try:
            ...
            raise cfg_error("[mdf_loadcell_probe] requires the NumPy module")

        self._printer = config.get_printer()

        # Sensor types supported by mdf_loadcell_probe.
        sensors = {}
        ...
        # Base load-cell object. This provides calibration/status and owns the
        # sensor stream used by the MCU load-cell probe object.
        self._load_cell = load_cell.LoadCell(config, sensor)

        # Configuration helpers required by McuLoadCellProbe.
        config_helper = LoadCellProbeConfigHelper(config, self._load_cell)

        # Consume x_offset/y_offset/z_offset from the config so Klipper does
        # not reject the section. MDF does not register this object as the
        # official probe, but keeping the offset helper is harmless and preserves
        # config compatibility with the original load_cell_probe section.
        self._probe_offsets = probe.ProbeOffsetsHelper(config)
        self._mcu = self._load_cell.get_sensor().get_mcu()

        # IMPORTANT: create TriggerDispatch here, during config loading, and on
        # the same MCU as the load-cell sensor. Creating it later from MDF live
        # code is too late for Klipper's MCU/trsync configuration phase.
        trigger_dispatch = mcu.TriggerDispatch(self._mcu)

        continuous_tare_filter_helper = ContinuousTareFilterHelper(
            config,
            sensor,
            trigger_dispatch.get_command_queue()
        )

        # MDF only needs this backend object: it creates the MCU oid, configures
        # the firmware-side load_cell_probe object, attaches the HX711 stream,
        # and owns the TriggerDispatch/trsync bridge.
        ...
        # Deliberately do NOT create/register:
        #   - ProbeParameterHelper
        #   - LoadCellProbingMove
        #   - TappingMove / TapSession / ProbeSessionHelper
        #   - LoadCellProbeCommands (LOAD_CELL_PROBE / LOAD_CELL_TEST_TAP)
        #   - ProbeVirtualEndstopDeprecation
        # Cartographer remains the official Klipper probe. This object is only
        # the MDF raw/trsync provider.
        self._printer.add_object('mdf_loadcell_probe_real', self)

    def get_mcu_load_cell_probe(self):
        return self._mcu_load_cell_probe

    def get_offsets(self):
        return self._probe_offsets.get_offsets()

    def get_status(self, eventtime):
        return self._load_cell.get_status(eventtime)
